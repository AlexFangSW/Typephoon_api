...
class AuthService:
    ...
    async def login_redirect(
        ...
        try:
            ...
            async with self._sessionmaker() as session:
                ...
                ret = await user_repo.register(
                    id=handle_auth_ret.user_id, name=handle_auth_ret.username
                )
    ...
    async def token_refresh(self, refresh_token: str) -> ServiceRet[str]:
        ...
        try:
            ...
            logger.warning(
                "invalid token, token: %s, error: %s", refresh_token, str(ex)
            )
